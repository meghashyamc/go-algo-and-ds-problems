...
func getKthNodeFromMiddle(head *listNode, k int) int {

	nodesCount := getNodesCountForLinkedList(head)

	positionOfKthNode := (nodesCount / 2) - k
	if positionOfKthNode < 0 {
		return -1
	}
	currNode := head
	for i := 0; i < positionOfKthNode; i++ {
...
func getNodesCountForLinkedList(head *listNode) int {
